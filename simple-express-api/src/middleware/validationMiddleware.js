...
const preventNoSQLInjection = (req, res, next) => {
  const checkForInjection = (obj) => {
    ...
    for (const key in obj) {
        ...
        // Якщо ключ містить оператор Mongo ($...) або крапку (dot notation) — вважаємо запит підозрілим
        if (typeof key === 'string' && (key.startsWith('$') || key.includes('.') || key.includes('$'))) {
          return res.status(400).json({
            success: false,
            message: 'Потенційно небезпечний запит (недопустимі ключі)'
          });
        }
      ...
      if (typeof value === 'string') {
        ...
        // Масиви або вкладені об'єкти — перевіряємо рекурсивно
